...
static int pmw3610_async_init_configure(const struct device *dev) {
    ...
    // cpi (runtime-adjustable)
    if (!err) {
        err = set_cpi(dev, pmw3610_rt_cpi());
    }
...
static void activate_automouse_layer() {
    ...
    k_timer_start(&automouse_layer_timer, K_MSEC(pmw3610_rt_automouse_timeout_ms()), K_NO_WAIT);
}
...
static int pmw3610_report_data(const struct device *dev) {
    ...
    switch (input_mode) {
    case MOVE:
        set_cpi_if_needed(dev, pmw3610_rt_cpi());
        dividor = pmw3610_rt_cpi_dividor();
        break;
    case SCROLL:
        set_cpi_if_needed(dev, pmw3610_rt_cpi());
    ...
    if (x != 0 || y != 0) {
        if (input_mode == MOVE || input_mode == SNIPE) {
            ...
            if (abs(data->scroll_delta_y) > pmw3610_rt_scroll_tick()) {
                input_report_rel(dev, INPUT_REL_WHEEL,
                                 data->scroll_delta_y > 0 ? PMW3610_SCROLL_Y_NEGATIVE : PMW3610_SCROLL_Y_POSITIVE,
                                 true, K_FOREVER);
                data->scroll_delta_x = 0;
                data->scroll_delta_y = 0;
            } else if (abs(data->scroll_delta_x) > pmw3610_rt_scroll_tick()) {
